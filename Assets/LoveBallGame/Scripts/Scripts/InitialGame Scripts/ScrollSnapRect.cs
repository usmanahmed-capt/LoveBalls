...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    private int CurrentPageonOnEnable;
    private bool CanMoveOnDragEnd;
    private bool FirtTimeDone;
    public ScrollSnapRect scrollSnapRect;
    public int PageInduxCallingFun;
    //------------------------------------------------------------------------
    ...
    private void OnEnable()
    {
        ...
        CanMoveOnDragEnd = true;
        if (IsUpScroller)
        {
            int CurrentPageonOnEnable = GetNearestPage();
            //  _currentPage = scrollSnapRect._currentPage + scrollSnapRect._currentPage;
            _currentPage = PageInduxCallingFun /*+ PageInduxCallingFun*/;
        ...
        {
            int CurrentPageonOnEnable = GetNearestPage();
            if (scrollSnapRect._currentPage / 2 == 0)
            {
                // Debug.LogError(scrollSnapRect._currentPage);
                _currentPage = scrollSnapRect._currentPage; /*- (scrollSnapRect._currentPage / 2);*/
                // GameController.Instance.currenLvlMain = _currentPage;
                // GameController.Instance.currenLvlMain++;
                LerpToPage(_currentPage);
            }
            else
            {
                int prevousePageIndux = scrollSnapRect._currentPage; /*- 1;*/
                // Debug.LogError(prevousePageIndux);
                _currentPage = prevousePageIndux; /*- (prevousePageIndux / 2);*/
                //  GameController.Instance.currenLvlMain = _currentPage;
                //  GameController.Instance.currenLvlMain++;
                LerpToPage(_currentPage);
            }
    ...
    private void LerpToPage(int aPageIndex) {
        ...
        _lerp = true;
        _currentPage = aPageIndex;
        scrollSnapRect._currentPage = _currentPage;
    }
    ...
    private void NextScreen() {
        ...
        CanMoveOnDragEnd = false;
        LerpToPage(_currentPage + 1);
        //Debug.LogError(_currentPage);
        //Debug.LogError(_pageCount);
        if (_currentPage == _pageCount-1)
            nextButton.SetActive(false);
    ...
    public void PageIndux(int pageInux) 
    {
        PageInduxCallingFun = pageInux;
    }
}
