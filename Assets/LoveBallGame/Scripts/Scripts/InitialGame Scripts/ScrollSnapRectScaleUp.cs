...
public class ScrollSnapRectScaleUp : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    public List<Image> _pageSelectionImages;
    public List<Transform> ContenClid = new List<Transform>();
    public int ShopIndux;
    ...
    void Start() {
        ...
        if (ContenClid.Count == 0)
        {
            for (int i = 0; i < _pageCount; i++)
            {
                ContenClid.Add(_container.GetChild(i));
            }
        }
    ...
    void Update() {
        ...
        for (int i = 0; i < ContenClid.Count; i++)
        {
            ContenClid[i].localScale = Vector3.one;
        }
        ContenClid[_currentPage].transform.localScale = Vector2.one * 1.3f;
